...
import numpy as np, os, sys, pandas as pd, csv, datetime, pickle as pkl, random, matplotlib.pyplot as plt
...
def sanity_check(opt):
    """
    Quick check to see if input values make sense (e.g. existent for both tasks).

    Inputs:
        opt: Namespace, contains all training parameters
    """
    assert len(opt.cs_per_bs)==len(opt.embed_sizes)==len(opt.losses)==len(opt.sampling)
    assert len(opt.embed_sizes)==len(opt.proxy_lr)==len(opt.beta_lr)==len(opt.margin)==len(opt.beta)==len(opt.nu)==len(opt.task_p)
    assert len(opt.adversarial)==len(opt.adv_weights)
...
"""============================================================================================================="""
### COMPUTE EVALUATION METRICS FOR CUB200, CARS196, SOP AND single PKU-TESTSETS
def eval_metrics_one_dataset(model, test_dataloader, device, k_vals=[1,2,4,8], evaltype='Class'):
    """
    ...
    Args:
        ...
        evaltype:           Name of embedding to use for evaluation.
    Returns:
        F1 score (float), NMI score (float), recall_at_k (list of floats), data embedding (np.ndarray)
    """
    ...
    with torch.no_grad():
        ...
        final_iter = tqdm(test_dataloader, desc='Computing {} Evaluation Metrics...'.format(evaltype.upper()))
...
"""============================================================================================================="""
### COMPUTE EVALUATION METRICS FOR IN-SHOP CLOTHES ###############
def eval_metrics_query_and_gallery_dataset(model, query_dataloader, gallery_dataloader, device, k_vals=[1,10,20,30,50], evaltype='Class'):
    """
    Compute evaluation metrics on test-dataset, e.g. NMI, F1 and Recall @ k.
...
def recover_closest_one_dataset(feature_matrix_all, image_paths, save_path, n_image_samples=10, n_closest=3):
    """
    Provide sample recoveries. Mainly acts as a sanity check to see if the model is actually learning to recover.
...
def recover_closest_inshop(query_feature_matrix_all, gallery_feature_matrix_all, query_image_paths, gallery_image_paths, save_path, n_image_samples=10, n_closest=3):
    """
    Provide sample recoveries specifically for the In-Shop Clothes dataset.
...
"""================================================================================================="""
### Provides a list of default metrics to use with a given dataset.
...
"""================================================================================================="""
### Relevant cluster functions to use in combination with training the aux. interclass embedding
...
def deepcluster(opt, dataloader, model, num_cluster):
    _ = model.eval()
    with torch.no_grad():
        features = []
        final_iter = tqdm(dataloader, desc='Computing DeepCluster-Embeddings')
        for idx, inp in enumerate(final_iter):
            input_img = inp[-1]
            out = model(input_img.to(opt.device))['Shared']
            features.extend(out.cpu().detach().numpy().tolist())
        features = np.vstack(features).astype('float32')
        torch.cuda.empty_cache()

        cluster_assignments = run_kmeans(features, num_cluster)
    return cluster_assignments

def initcluster(opt, dataloader, model, num_cluster):
    #Compute features
    _ = model.eval()
    with torch.no_grad():
        features = []
        ...
        for idx,inp in enumerate(final_iter):
            input_img = inp[-1]
            out = model(input_img.to(opt.device), is_init_cluster_generation=True)
            features.extend(out.cpu().detach().numpy().tolist())
        features = np.vstack(features).astype('float32')
    ...
    if opt.cluster_mode=='mstd' and opt.arch!='googlenet':  stds = np.stack([np.std(features[classes==i,:],axis=0) for i in np.unique(classes)])
    for i in range(len(features)):
        features[i,:] -= means[classes[i],:]
        if opt.cluster_mode=='mstd' and opt.arch!='googlenet': features[i,:] /= stds[classes[i],:]

    n_samples, dim  = features.shape
    image_paths     = np.array([x[0] for x in dataloader.dataset.image_list])

    #Cluster each dataset
    cluster_labels = run_kmeans(features, num_cluster)
